...
def search_faiss_index(index, vector, records, instructor_name=None, course_type=None, duration=None, time=None,credits = None, top_k=3):
...
def main():
    st.title("Optimal Search Engine")

    # Get the directory of the current script
    script_dir = Path(__file__).parent

    # Construct the relative paths
    db_file = script_dir / "../data/db/courses.sqlite"
    faiss_index_file = script_dir / "../data/faiss_index/faiss_index.idx"

    # Convert to absolute paths and ensure they are strings
    db_file = str(db_file.resolve())
    faiss_index_file = str(faiss_index_file.resolve())
    ...
    try:
        ...
        credit = [record[5] for record in records]
        ...
        if user_input:
            ...
            recommended_credits = recommend_input(user_input, credit)

            if 'show_hint' not in st.session_state:
                st.session_state.show_hint = False

            # Button to toggle the hint display
            if st.button("Give me the Hint!"):
                st.session_state.show_hint = not st.session_state.show_hint  # Toggle the state


            # Conditionally display the hint content
            if st.session_state.show_hint:
                st.write(f"Recommended Instructors: {', '.join(recommended_instructors)}")
                st.write(f"Recommended course type: {', '.join(recommended_course_types)}")
                st.write(f"Recommended Durations: {', '.join(recommended_durations)}")
                st.write(f"Recommended Times: {', '.join(recommended_times)}")
                st.write(f"Recommended credits: {', '.join(recommended_credits)}")
            ...
            instructor_name, course_type, duration, time,credits = None, None, None, None,None
            ...
            for facet in facet_values:
                ...
                elif "credits" in facet:
                    credits = correct_spelling(facet, credit)
                elif "course type" in facet:
            ...
            results = search_faiss_index(index, user_vector, records, instructor_name=instructor_name,
                                         course_type=course_type, duration=duration, time=time,credits=credits, top_k=3)
